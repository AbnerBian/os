...
int DirComd(int k)	//dir命令，显示指定目录的内容（文件名或目录名等）
{
    ...
    while (s>0)
    {
        ...
        for (i=0;i<4;i++,p++)
        {
            ...
            cout<<resetiosflags(ios::right);     //每次设置左对齐或者右对齐时都应该重置
...
int CopyComd(int k)		//copy命令的处理函数：复制文件
{
    ...
    if(*FileName2=='.'&&gFileName[1]!=*FileName1)
    {
   ...
   else if (!IsName(FileName2))		//若名字不符合规则
    {
        cout<<"\n命令中的目标文件名错误。\n";
        return -2;
    }
...
int DelComd(int k)			//del(删除文件)命令处理函数
{
    ...
    if(comd[1][0]=='*')
    {
        //罗列所有文件名，删除这些文件，如果文件是只读属性，那么询问用户是否删除，用户选择删除，则删除
        if(k>=2||k==0)
        {
            cout<<"命令参数";
        }
...
int FcComd(int k)
{
    ...
    //读取两个文件存在两个数组，对数组内的内容进行比较
    //compare(array1,array2)
    //定义需求的变量
    short i,j,i_uof,ii,error,pos,offset;
    char attrib='\0';
    short b,b0,bnum,count=0,readc;
    ...
    //第一个文件
    error=FindPath(comd[1],attrib,0,fcbp);        //构成全路径且去掉".."存于temppath
    if(error==-1)
    {
        cout<<"\n路径错误";
        return -1;
    ...
    {
        //打开
        Put_UOF(temppath,i_uof,2,fcbp);
        strcpy(FileName1,temppath);
        cout<<"\n文件"<<FileName1<<"打开成功。\n";
        if(uof[i_uof].readp==0)
        {
            cout<<"\n文件"<<temppath<<"是空文件。\n";
    ...
    readc=uof[i_uof].fsize-pos+1;   //读到文件尾部共需读readc个字节
    bnum=(pos-1)/SIZE;      //从文件的第bnum块读（bnum从零开始编号）
    offset=(pos-1)%SIZE;    //在第bnum块的偏移位置offset处开始读  （offset从0开始的）
    b=uof[i_uof].faddr;     //取文件的首块号
    for(i=0;i<bnum;i++) //寻找读入的第一个盘块号
    {
        b0=b;
        b=FAT[b];
    ...
    error=FindPath(comd[2],attrib,0,fcbp);        //构成全路径且去掉".."存于temppath
    if(error==-1)
    {
        cout<<"\n路径错误";
        return -1;
